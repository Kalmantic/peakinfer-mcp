...
import { readFileSync, readdirSync, statSync, existsSync, readFile } from 'fs';
...
function walkDir(dir: string, basePath: string, files: CodeFile[], totalSize: { value: number }): void {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (shouldIgnoreFile(entry.name) || !isCodeFile(entry.name)) continue;
...
export async function readCodeFiles(targetPath: string): Promise<CodeFile[]> {
