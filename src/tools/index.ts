...
function formatError(error: MCPError): { content: Array<{ type: string; text: string }> } {
  const lines = [
...
async function handleGetBenchmark(args: Record<string, unknown>) {
  const model = args.model as string;
  const framework = (args.framework as string) || 'api';
  const hardware = (args.hardware as string) || 'api';
...
async function handleCompareToBaseline(args: Record<string, unknown>) {
  const currentAnalysis = args.current_analysis as Record<string, unknown>;
  const baselinePath = args.baseline_path as string | undefined;
  ...
  const currentCallsites = (currentAnalysis.callsites as unknown[])?.length || 0;
  const baselineCallsites = baseline ? (baseline.callsites as unknown[])?.length || 0 : 0;
...
async function handleListTemplates(args: Record<string, unknown>) {
  ...
  const templates: Array<{ name: string; category: string }> = [];
...
async function handleGetTemplate(args: Record<string, unknown>) {
...
async function handleSaveAnalysis(args: Record<string, unknown>) {
  const analysis = args.analysis as Record<string, unknown>;
