...
function normalizeHeliconeEvent(req) {
...
export async function fetchHeliconeEvents(config) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new ConnectorError(`Helicone API error: ${response.status} ${errorText}`, 'helicone', response.status);
        }
        ...
        throw new ConnectorError(`Failed to fetch from Helicone: ${error instanceof Error ? error.message : 'Unknown error'}`, 'helicone');
    }
}
